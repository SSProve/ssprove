...
Module PackageTheory (π : ProbRulesParam).
  ...
  *)
  ...
  Section ID.
    ...
    Proof.
      exists fset0.
      (* Should probably map the set to a map *)
      (* move => oo Hin aa.
      apply (opr oo Hin aa).
      move => fx.
      apply ret.
...
Module Example (π : ProbRulesParam).

  Module TH := (PackageTheory π).
  Import TH.

  Definition Game_import : Interface := fset0.

  Definition Game_Type (Game_export : Interface) : Type :=
    package_type Game_import Game_export.

  Open Scope fset.

  Let GEN := (1, (chUnit, chUnit)).
  Let SET := (2, (chBool, chUnit)).
  Let GET := (3, (chUnit, chBool)).

  Definition KEY_export : Interface := [fset GEN; SET; GET].

  Definition CK0_import : Interface := [fset SET].

  Definition CK1_import : Interface := [fset GEN].

  Context (KEY : package_type fset0 KEY_export).
  Context (CK_export : Interface).
  Context (CK0 : package_type CK0_import CK_export).
  Context (CK1 : package_type CK1_import CK_export).

  Canonical fset_unlockable T := [unlockable of @fset T].

  Ltac unfold_sets :=
    match goal with
    | |- context [ ?A :|: ?B ] =>
      first [ unfold A | unfold B ]
    | |- context [ fsubset ?A ?B ] =>
      first [ unfold A | unfold B ]
    end.
  ...
  Ltac fset_unlock :=
    repeat unfold_sets ;
    repeat (unfold ":|:", fsubset) ;
    rewrite unlock ;
    exact isT.

  Notation "🔒" := (ltac:(abstract fset_unlock)).

  Definition K0 : package_type _ _ :=
    link' (par CK0 (ID [fset GET])) KEY 🔒.

  Definition K1 : package_type _ _ :=
    link' (par CK1 (ID [fset GET])) KEY 🔒.

  Definition CD_import : Interface := [fset GET].
  Context (CD_export : Interface).
  Context (CD0 : package_type CD_import CD_export).
  Context (CD1 : package_type CD_import CD_export).

  Definition D0 : package_type _ _ :=
    link' (par (ID [fset GEN]) CD0) KEY 🔒.

  Definition D1 : package_type _ _ :=
    link' (par (ID [fset GEN]) CD1) KEY 🔒.

  Check (link' (par CK0 CD0) KEY 🔒).

  (* Definition A_export : Interface := fset (cons (0, (chUnit, chBool)) nil). *)

  (* Definition Adversary4Game (Game_export : Interface) : Type := *)
  (*   package_type Game_export A_export. *)

  Let iops_StP := @ops_StP probE rel_choiceTypes chEmb.
  Let iar_StP := @ar_StP probE rel_choiceTypes chEmb.

  Definition dommHeap (s : {fset Locations}) := { x : Locations | x \in s }.

  Definition makeHeap (s : {fset Locations}) :=
    {fmap (dommHeap s) → Values}.

  Definition makeHeap_cT (s : {fset Locations}) :=
    [choiceType of (makeHeap s)].

  Definition Game_op_import_S : Type := {_ : Names & void}.
  Definition Game_import_P : Game_op_import_S → choiceType :=
    fun v => match v with
            existT a b => match b with end
          end.

  Definition getFromMap
    {locs : {fset Locations}}
    (map : {fmap {x : Locations | x \in locs} → Values})
    (l : Locations) (H : l \in locs) : Values.
  Proof.
  ...
  Definition setFromMap
    {locs : {fset Locations}}
    (map : {fmap {x : Locations | x \in locs} → Values})
    (l : Locations) (H : l \in locs) (v : Values) : {fmap {x : Locations | x \in locs} → Values}.
  Proof.
    apply (setm map).
    - exists l. exact H.
    - exact v.
  ...
  Definition fromEmpty {B} {v : Names} (H : v \in fset0) : B.
  ...
  Fixpoint FreeTranslate {B : choiceType} {locs : {fset Locations}}
             (v : raw_program locs Game_import B) : rFreeF (iops_StP (makeHeap_cT locs)) (iar_StP (makeHeap_cT locs)) B
    :=
    match v with
    | ret a => FreeProbProg.ret _ _ _ a
    | opr oo Hin aa k  => fromEmpty Hin
    | sampler op k' => FreeProbProg.opr _ _ _ (inr op) (fun w => FreeTranslate (k' w))
    | getr l Hin k' => FreeProbProg.opr _ _ _ (inl (inl (gett _))) (fun w => FreeTranslate (k' (getFromMap w l Hin)))
    | putr l Hin v k' => FreeProbProg.opr _ _ _ (inl (inl (gett (makeHeap_cT locs))))
                 (fun w => FreeProbProg.opr _ _ _ (inl (inr (putt (makeHeap_cT locs) (setFromMap w l Hin v))))
                                                  (fun w' => FreeTranslate k'))
    end.

  Definition Pr (P : package_type Game_import A_export) : SDistr (bool_choiceType).
  Proof.
    destruct P as [locs PP].
    pose STDIST := thetaFstd _ (FreeTranslate (PP (0, (chUnit, chBool)) _ _)).
    simpl in STDIST.
    pose SSDIST := STDIST prob_handler _ Datatypes.tt emptym.
    refine (SDistr_bind _ _ (fun '(b, _) => SDistr_unit _ b) (SSDIST _)).
    rewrite in_fset.
    intuition.
  Defined.
  ...
  Proof.
    move => A.
    assert (AdvantageE F G A = ϵ1 A) as Ineq1. { rewrite ineq1. reflexivity. }
    assert (AdvantageE G H A = ϵ2 A) as Ineq2. { rewrite ineq2. reflexivity. }
    assert (AdvantageE F H A = ϵ3 A) as Ineq3. { rewrite ineq3. reflexivity. }
  ...
  Lemma Reduction {Game_export M_export : Interface} {M : package_type Game_export M_export}
        (G : GamePair Game_export) (A : Adversary4Game M_export) (b : bool) :
    `| Pr (A ∘ (M ∘ G b)) true | = `| Pr ((A ∘ M) ∘ G b)  true |.
  Proof.
    rewrite link_assoc.
    reflexivity.
  Qed.

  (* ER: I couldn't make sense of the types for the corollary *)
  Corollary Reduction' {Game_export M_export : Interface} {M : package_type Game_export M_export}
            (G : GamePair Game_export) {A : Adversary4Game M_export} :
    (A ∘ M ∘ G false) ≈[ (fun A' => ϵ(G) (A' ∘ A ∘ M)) ] (A ∘ M ∘ G true).
  Proof.
    simpl.
    extensionality A'.
    rewrite /AdvantageE.
