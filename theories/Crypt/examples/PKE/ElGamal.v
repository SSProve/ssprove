...
Lemma OT_CPA_elgamal {A} `{IsAdversary (I_CPA elgamal) A} :
...
Notation OTM n := (SLIDE elgamal n%N ∘ (ID (I_CPA elgamal) || RAND (unif n%N)))%sep.

Lemma MT_CPA_elgamal n {A} `{IsAdversary (I_CPA elgamal) A} :
