...
(*     (* have prgL : program (rel_loc :|: rel_loc) Game_import ('fin (2 ^ n)). *) *)
(*     (* { apply: getr. { exact: key_location_in_rel_loc. } *) *)
...
(*     (* have hfoo : prgL = *) *)
(*     (*             ((exist (valid_program (IND_CPA_pkg_ff.π1 :|: ENC_pkg.π1) Game_import) *) *)
(*     (*   (_getr key_location *) *)
(*     (*      (λ x : option nat, *) *)
(*     (*         raw_program_link *) *)
(*     (*           (sval *) *)
...
(*           repeat program setoid fold. simpl. *)
...
(*             } *)
(*             1:{ *)
(*               (* now the two programs differ in some dead code in the RHS *) *)
(*               simpl. *)
(*               apply: rdead_sampler_eliminationL. *)
