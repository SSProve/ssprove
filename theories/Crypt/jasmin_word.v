...
Definition wlt {sz} (sg: signedness) : word sz -> word sz -> bool :=
  match sg with
  | Unsigned => fun x y=> (urepr x < urepr y)%R
  | Signed => fun x y=> (srepr x < srepr y)%R
  end.
...
Lemma eq_from_wbit_n s (w1 w2: word s) :
  reflect (forall i : 'I_(wsize_size_minus_1 s).+1, wbit_n w1 i = wbit_n w2 i) (w1 == w2).
...
Definition truncate_word s s' (w:word s') : exec (word s) :=
  ...
  | Eq => fun h => ok (ecast s (word s) (esym (cmp_eq h)) w)
  | Lt => fun _ => ok (zero_extend s w)
  | Gt => fun _ => type_error
  end erefl.
...
Definition wpshufd_256 (s : u256) (o : Z) : u256 :=
  make_vec U256 (map (fun w => wpshufd_128 w o) (split_vec U128 s)).
...
Definition wpshufl_u64 (w:u64) (z:Z) : u64 :=
  ...
  make_vec U64 (map (fun n => v`_(Z.to_nat (urepr n)))%R j).
...
Definition wpshufl_u256 (s:u256) (z:Z) :=
  make_vec U256 (map (fun w => wpshufl_u128 w z) (split_vec U128 s)).

Definition wpshufh_u256 (s:u256) (z:Z) :=
  make_vec U256 (map (fun w => wpshufh_u128 w z) (split_vec U128 s)).
...
Definition wpblendd sz (w1 w2: word sz) (m: u8) : word sz :=
  ...
  let r := map3 (fun b v1 v2 => if b == 1%R then v2 else v1) b v1 v2 in
  make_vec sz r.
...
Definition wpermq (w: u256) (i: u8) : u256 :=
  ...
  make_vec U256 (map (fun n => v`_(Z.to_nat (urepr n)))%R j).

(* -------------------------------------------------------------------*)
Definition wpsxldq op sz (w: word sz) (i: u8) : word sz :=
  let n : Z := (Z.min 16 (wunsigned i)) * 8 in
  lift1_vec U128 (fun w => op w n) sz w.
...
Section FORALL_NAT_BELOW.
  ...
  Lemma forallnat_belowP n :
    reflect (forall i, (i < n)%coq_nat -> P i) (forallnat_below n).
  Proof.
    elim: n.
...
(*         forallnat_below (fun i : nat => wbit_n k i == (n <= i)%nat) (wsize_size_minus_1 sz).+1 *)
(*       ). *)
...
(*   apply: ltZE; rewrite mulZE !rmorphM/= ltr_pM2r//. *)
(*   by rewrite -[ltRHS]/(Z_to_int 18446744073709551616); apply/ltZE. *)
(* Qed. *)
